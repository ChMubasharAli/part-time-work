...
export async function GET(request: NextRequest) {
  try {
    ...
    const limit = parseInt(searchParams.get("limit") || "50"); // Load more records at once
    const skip = (page - 1) * limit;

    // Get patients with pagination (only personal info)
    const [patients, totalCount] = await Promise.all([
      prisma.task9PatientPersonal.findMany({
        skip,
        take: limit,
        orderBy: { id: "asc" },
        select: {
      ...
      prisma.task9PatientPersonal.count(),
    ]);
    ...
    console.error("Error fetching Task9 patients:", error);
    return NextResponse.json(
      {
        success: false,
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (mode === "create") {
      // Create patient in all 4 tables using transaction
      ...
      return NextResponse.json({
        success: true,
        message: "Patient record created successfully in all 4 tables!",
        patientId: result.id,
      });
    } else {
      return NextResponse.json(
    ...
    // Handle unique constraint violation (CNIC already exists)
    if (error.code === "P2002") {
      return NextResponse.json(
        {
          success: false,
